...
## 1 ##
seeds_df_long = pivot_longer(data = seeds_df, cols = c("a", "p", "c", "l_k", "w_k", "asym", "l_g"), 
                             names_to = "measurement", values_to = "value")

ggplot(seeds_df_long, aes(x = value, color=variety)) +
  geom_histogram() +
...
```{r}
  ## 2 ##
ggplot(data = seeds_df, aes(x = a, y = asym, color = variety)) + 
  geom_point() + 
  theme_bw() + 
  labs(x = "kernel area", y = "assymetry coefficient")
```

```{r}
  ## 3 ##
ggplot(data = seeds_df, aes(x = l_k, y = w_k, color = variety)) + 
  geom_point() + 
  theme_bw() + 
  labs(x = "kernel groove length", y = "kernel width")
...
```{r}
#| fig-cap: we would use 3 groups to initiate a kmeans cluster because the kink in the graph is most pronounced at three clusters.
...
set.seed(10101)
seeds_km <- kmeans(x = seeds_scale, centers = 3, nstart=25) # kmeans specifying 3 groups to start
```
...
ggplot(data = seeds_cl, aes(x = a, y = asym, color = cluster_no, shape=variety)) + 
  geom_point() + 
  theme_bw() + 
  labs(x = "kernel area", y = "assymetry coefficient")
...
3) select only numeric variables

4) scale the data

5) Get distance of the scaled data

6) Run hclust

7) Linkages either single or complete (or another linkage)
...
p_complete<-ggdendrogram(hc_complete, 
             rotate = TRUE) +
  theme_minimal() +
  labs(x = "Country", title = "Complete Cluster")
...
p_single<-ggdendrogram(hc_single, 
             rotate = TRUE) +
  theme_minimal() +
  labs(x = "Country", title = "Single Cluster")
